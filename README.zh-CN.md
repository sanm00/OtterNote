...
- 计划 / 笔记 / 回顾 三视图工作区，侧边导航
- 带日程与优先级设置的周计划待办
- 回顾小结：新建笔记、完成待办与每周柱状图
- Markdown 编辑与实时预览，代码块语法高亮
- 从笔记正文中内联解析 `- [ ]` 待办
- 标签行（`#标签`）与维基链接（`[[标题]]`）自动建立反向链接
- 全局搜索笔记与待办内容
- 通过文件选择器或粘贴上传图片，桌面端分别存储预览图与原图
- 笔记导出 Markdown，JSON 备份导入/导出（可自选存储路径）
- 浅色与深色主题
- 可自定义的键盘快捷键（⌘N、⌘T、⌘S、⌘E …）
- 笔记置顶小窗
...
**计划页**（浅色）——左侧为快速待办栏的周计划视图。

![计划页](docs/screenshots/plan.png)

**笔记与编辑**——笔记列表与 Markdown 编辑，包含代码块、标签与反向链接。

![笔记与编辑](docs/screenshots/notes.png)

**回顾页**（浅色）——每周小结，以及新建笔记与完成待办的柱状图。

![回顾页](docs/screenshots/review.png)
