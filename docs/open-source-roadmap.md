...
- [x] 移除或重新评估 `package.json` 中的 `private` 字段。（根 `package.json` 保持 `private: true`：它不是待发布的包，`npm pack` 会打包约 9.0 MB、61 个文件的源码与测试。面向用户的 CLI 源文件在 `release/npm/package/`，发布时由 `release/npm/assemble.mjs` 组装出包名 `otter-note`、含 `files` 白名单的 npm 包，不做二次跟踪）
...
- `release/npm/package`（发布为包名 `otter-note`）已实现 `npm install -g otter-note` 一条命令：安装时下载、校验并安装桌面应用，另提供 `otter-note install|uninstall|launch|--version`。为避免两套安装逻辑漂移，包内不重写逻辑：`release/npm/assemble.mjs` 在发布前把 `release/npm/package/` 的包源码（`bin/`、`lib/`、README）和 `release/install/install.sh` 与仓库根 LICENSE 组装进生成的包，postinstall 调用包内安装器；声明了 Node.js 版本、`os`/`cpu` 支持范围，不支持平台时只告警、不让 `npm install` 失败。已用真实 tarball 完成 21 项端到端验证（`npm pack` 内容白名单、临时 prefix 全局安装、postinstall 完成安装、CLI 各子命令、卸载需确认、`OTTERNOTE_SKIP_INSTALL`、以及 `/Applications` 未被改动）。启动路径顺带消除一个潜伏竞态：原先 `spawn` 之后立刻 `process.exit()`，可能丢掉启动；现在等待 `spawn` 事件再 `unref`，并以 `process.exitCode` 退出。
- npm 发布闸门：`release/npm/verify.mjs` 用 23 项断言固化两条渠道的对齐关系（版本号一致、根包保持 `private`、GitHub slug 一致、`os`/`cpu` 与 Windows 拒绝、`lib/platform.js` 的资产名与 `install.sh` 的 `asset_candidates()` 逐字一致（macos/linux × 架构共 4 组，用哨兵版本 `0.0.0-consistency-check` 比对）、macOS 镜像名跟随 `tauri.conf.json` 的 `productName`、默认安装目录规则一致（`/Applications` 与 `XDG_BIN_HOME`→`~/.local/bin`）、`files` 白名单齐全、`bin` 带 shebang、零依赖、npm 发布唯一入口 `publish.sh` 已接线执行闸门与组装）。npm 包不再纳入仓库跟踪，闸门在临时目录里先跑 `release/npm/assemble.mjs` 再校验生成的包内容，因此任何漂移都会在 PR 上红。`release/npm/publish.sh` 负责编排：工作区检查 → 一致性闸门 → 全量质量门 → 确认对应 GitHub release 已存在（`postinstall` 要下载它；缺失时真实发布会中止，`--dry-run` 只提示）→ 组装包到 `.npm-pkg/` → 展示 tarball → 确认后 `npm publish`；它从不改写版本号（唯一真源是 `src-tauri/tauri.conf.json`），也不打 tag。实测：在仓库镜像上注入 7 种漂移（版本不一致、根包取消 private、安装脚本资产名改动、默认安装目录改动、`publish.sh` 未接线闸门、GitHub slug 改动、`bin` 文件缺失）全部以 exit 1 被拒绝并指出具体断言，还原后复通；`sh release/npm/publish.sh --dry-run --yes --skip-gates` 全链跑通（tarball 6 文件 / 7.5 kB，正确报告 `v0.1.0` release 尚不存在）。`npm view otter-note` 返回 404，包名可用。用法见 `CONTRIBUTING.md` 的「Releasing」与 `release/npm/package/README.md`。
